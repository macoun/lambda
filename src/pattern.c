...
static expr pattern_walk_vars(struct machine *m, expr pattern, int depth, expr literals, expr bindings)
{
  ...
  else if (is_pair(pattern))
  {
    ...
    while (i < len)
    {
      ...
      if (is_ellipsis_pattern(cur))
      {
        ...
        bindings = pattern_walk_vars(m, var, depth + 1, literals, bindings);
        cur = cdr(cdr(cur)); // Skip var and ellipsis
        i += 2;
      }
      else
      {
        bindings = pattern_walk_vars(m, var, depth, literals, bindings);
        cur = cdr(cur);
        i++;
      }
    }
...
{
  expr pvars = pattern_walk_vars(m, pattern, 0, literals, NIL);
  return pvars;
}
