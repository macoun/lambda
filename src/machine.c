...
#include "logger.h"
#include "printer.h"
#include "exp.h" // TODO remove the dependency
#include <stdlib.h>
...
struct cell machine_snapshot(struct machine *m)
{
  ...
  expr stack = vector(m, m->stack->size, m->stack->cells);
  expr regs = vector(m, m->registers->size, m->registers->cells);
  expr snap = cons(m, stack, regs);
...
{
  expr stack = car(snapshot);
  expr regs = cdr(snapshot);

  if (!is_vector(stack) || !is_vector(regs))
  {
  ...
  // Restore stack
  m->stack->size = vect_size(stack);
  for (size_t i = 0; i < m->stack->size; i++)
  {
    m->stack->cells[i] = vect_get(stack, i);
  }

  // Restore registers
  for (size_t i = 0; i < m->registers->size; i++)
  {
    m->registers->cells[i] = vect_get(regs, i);
  }
  // info("Restored machine snapshot. Stack size: %ld, Regs size: %ld", m->stack->size, m->registers->size);
